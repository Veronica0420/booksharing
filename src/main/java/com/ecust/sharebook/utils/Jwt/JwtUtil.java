package com.ecust.sharebook.utils.Jwt;
...
public class JwtUtil {
    ...
    public String createTokenByWxAccount(UserInf wxAccount) {
     ...
     //   redisTemplate.opsForValue().set("JWT-SESSION-" + jwtId, token, EXPIRE_TIME, TimeUnit.SECONDS);
        EhCacheUtil.init();
       // EhCacheUtil.put(EhCacheUtil.getKentrasoftCache(), jwtId, token, EXPIRE_TIME, EXPIRE_TIME);
        EhCacheUtil.put(EhCacheUtil.getKentrasoftCache(), jwtId, token,true);
    ...
    public boolean verifyToken(String token) {
        try {
          ...
          // String redisToken = redisTemplate.opsForValue().get("JWT-SESSION-" + getJwtIdByToken(token));
            String redisToken=EhCacheUtil.get(EhCacheUtil.getKentrasoftCache(),getJwtIdByToken(token)).toString();
